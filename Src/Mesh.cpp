...
namespace Mesh {
	...
	*/
	bool Buffer::Init(GLsizeiptr vboSize, GLsizeiptr iboSize)
	{
		...
		progStaticMesh = Shader::Program::Create("Res/Shader/StaticMesh.vert", "Res/Shader/StaticMesh.frag");
		...
		progTerrain = Shader::Program::Create("Res/Shader/Terrain.vert", "Res/Shader/Terrain.frag");
	...
	*/
		...
		const char* name, const Primitive& primitive, const Material& material)
	{
		if (files.find(name) != files.end()) {
			std::cerr << "[警告]" << __func__ << ":" << name << "という名前は既に追加されています" << std::endl;
			return false;
		}
		...
		std::cout << "[情報]" << __func__ << ":メッシュ" << name << "を追加" << std::endl;
		return true;
	}
	...
	*/
	FilePtr Buffer::GetFile(const char* name)const
	{
		...
		if (itr == files.end()) {
			std::cerr << "[警告]" << __func__ << ":" << name << "という名前のメッシュは追加されていません" << std::endl;
