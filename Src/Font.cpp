...
{
	return spriteRenderer.Init(maxCharacter,FILENAME_SHADER_VERT_SPRITE,FILENAME_SHADER_FRAG_SPRITE);
}
...
bool FontRenderer::LoadFromFile(const char* filename)
{
	...
	if (!fp) {
		std::cerr << "[Error]" << __func__ << ": " << filename << "を開けません" << std::endl;
		return false;
	}
...
bool FontRenderer::AddString(const glm::vec2& position,const wchar_t* str)
{
	...
	for (const wchar_t* itr = str; *itr; ++itr) {
		...
		if (info.id >= 0 && info.size.x &&info.size.y) {
			...
			//そこで,その差を撃ち消すための補正値を計算する.
			const float baseX = info.size.x *0.5 + info.offset.x;
			const float baseY = base - info.size.y *0.5f + info.offset.x;
