...
bool Window::GetCommand(COMMANDLIST command)
{
	...
	if (command==COMMANDLIST_DECISION) {
		//決定キーの処理,
		if (IsKeyDown(GLFW_KEY_ENTER) ||IsKeyDown(GLFW_KEY_SPACE) || gamepad.buttonDown & GamePad::A) {
	...
	else if (command == COMMANDLIST_MANUALMODE) {
		if (IsKeyDown(GLFW_KEY_ENTER) || gamepad.buttonDown & GamePad::START) {
			return true;
		}
	}
	else if (command == COMMANDLIST_DEBUGMODE) {
		if (IsKeyDown(GLFW_KEY_D) || gamepad.buttonDown & GamePad::X) {
			return true;
		}
	}
	else {
