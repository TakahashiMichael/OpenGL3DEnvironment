...
bool BufferObject::Create(GLenum target, GLsizeiptr size, const GLvoid* data, GLenum usage) {
	...
	return error == GL_NO_ERROR; //GLのエラーをそのまま返す
}
...
bool BufferObject::BufferSubData(GLintptr offset,GLsizeiptr size,const GLvoid* data)
{
	if (offset + size > this->size) {
		std::cerr << "[Warning]" << __func__ << "Destination area exceeds Buffer size" << std::endl;
		std::cerr << "buffer size:" << this->size << " offset:" << offset << " size" << size << std::endl;
		if (offset >= this->size) {
			return false;
		}
		//可能な範囲だけ転送を行う.
	...
	const GLenum error = glGetError();
	if (error != GL_NO_ERROR) {
		std::cerr << "[ERORR]" << __func__ << ":Failed to Transfer Data" << std::endl;
	}
...
bool VertexArrayObject::Create(GLuint vbo,GLuint ibo)
{
	...
	const GLenum error = glGetError();
	if (error!=GL_NO_ERROR) {
		std::cerr << "[ERROR]" << __func__ << ":Failed to Create VAO" << std::endl;
	}
	return error == GL_NO_ERROR;
}
